 ...
 *
 */
function MessagingStub_ClearwaterLocal() {
  this.ownRtcIdentity;
  this.credentials;
  ...
  this.socket;
  //this.initialWSUri = "ws://150.140.184.247:2701/";
 ...
 * @param message
 */
MessagingStub_ClearwaterLocal.prototype.sendMessage = function(message) {
...
MessagingStub_ClearwaterLocal.prototype.connect = function(ownRtcIdentity, credentials, callbackFunction, errorCallbackFunction) {
 ...
 */
MessagingStub_ClearwaterLocal.prototype.onSocketMessage = function(event, self) {
  var that = this;
...
MessagingStub_ClearwaterLocal.prototype.handleMessage = function(msg, wonderMsg, self) {
...
MessagingStub_ClearwaterLocal.prototype.disconnect = function() {
  if (this.socket)
    this.socket.close();
  this.socket = null;
...
MessagingStub_ClearwaterLocal.prototype.doSend = function(message) {
  var msgString = JSON.stringify(message);
  console.log("send message " + msgString);
  this.socket.send(msgString);
};

MessagingStub_ClearwaterLocal.prototype.ims2CloudMessage = function(message_command, status_code, attributes) {
  var msg = new Object();
